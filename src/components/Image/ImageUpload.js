...
const ImageUpload = ({
  ...
  return (
    <div className={`mt-4 ${containerClassName}`}>
      ...
      <label
        htmlFor={id}
        className={`text-sm font-medium w-auto text-black my-2 ${labelClassName}`}
      >
        {label}
      </label>
      ...
      {/* Dropzone */}
      <div
        className={`relative border bg-transparent border-black border-dashed text-black rounded-md shadow-sm p-4 focus:outline-none cursor-pointer flex items-center justify-center ${errorMessage ? "border-red-500" : ""
