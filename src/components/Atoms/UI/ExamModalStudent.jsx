...
const ExamModalStudent = ({ 
  ...
  useEffect(() => {
    if (isOpen && examData && isEditing) {
      ...
      console.log("Courses data available:", coursesData);
      console.log("Batches data available:", batchesData);
      
      // Extract duration
      const hours = examData.examduration?.hours || 0;
      const minutes = examData.examduration?.minutes || 0;
      
      // Handle questions - if not present, create empty question
      ...
      if (examData.questions && Array.isArray(examData.questions) && examData.questions.length > 0) {
        ...
        // If no questions in the data, add one empty question
        formattedQuestions = [{
          id: 1,
          text: '',
          options: { A: '', B: '', C: '', D: '' },
      ...
      // Get course and batch IDs from the exam data
      const courseId = examData.course?._id || examData.courseId || '';
      const batchId = examData.batch?._id || examData.batchId || '';
      
      // Find the actual course and batch objects from the dropdown data
      const selectedCourse = coursesData?.find(c => c._id === courseId) || examData.course || null;
      const selectedBatch = batchesData?.find(b => b._id === batchId) || examData.batch || null;
  ...
  }, [examData, isEditing, isOpen, coursesData, batchesData]);
  ...
  const filteredBatches = formData.courseId 
    ? batchesData?.filter(batch => {
        // Check if batch has courses array and includes the selected course
        if (batch.courses && Array.isArray(batch.courses)) {
          return batch.courses.includes(formData.courseId);
        }
        // Alternative: check batch.courseId
        const batchCourseId = batch.courseId || batch.course?._id;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const selected = filteredBatches?.find(b => b._id === e.target.value);
                  handleSelectChange('batchId', e.target.value, selected);
                }}
                className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
                disabled={isDisabled || !formData.courseId}
              >
              ...
              {filteredBatches?.length === 0 && formData.courseId && (
                <p className="text-sm text-yellow-500 mt-1">No batches available for this course</p>
              )}
            </div>
            ...
            <div className="md:col-span-2">
              <label className="flex items-center space-x-3">
                ...
                <span className="text-sm text-gray-500">(Draft exams won't be visible to students)</span>
              </label>
            </div>
          </div>
