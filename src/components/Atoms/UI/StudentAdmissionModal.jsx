...
const StudentAdmissionModal = ({
  ...
  useEffect(() => {
    if (initialData && initialData._id) {
      // Fetch installment details if needed
      const fetchInstallments = async () => {
        try {
          // If installments are just IDs, you might need to fetch them
          // For now, we'll set them as empty array if they're just IDs
          let installmentsData = [];
          if (initialData.installments && Array.isArray(initialData.installments)) {
            if (initialData.installments.length > 0 && typeof initialData.installments[0] === 'object') {
              installmentsData = initialData.installments.map((inst, idx) => ({
                id: inst._id || idx + 1,
          ...
          setFormData({
            ...
            dob: initialData.dateOfBirth ? initialData.dateOfBirth.split('T')[0] : initialData.dob ? initialData.dob.split('T')[0] : '',
          ...
          setCalculationFields({
            courseFees: initialData.fee || initialData.courseFees || '',
  ...
  // Auto-fill course fees when plan is selected
  useEffect(() => {
    if (formData.plan && applicablePlans.length > 0 && !initialData) {
      ...
      if (selectedPlan && selectedPlan.amount) {
        ...
        setCalculationFields(prev => ({
          ...prev,
          courseFees: planAmount
        }));
      }
    }
  }, [formData.plan, applicablePlans, initialData]);

  // Recalculate fees when discount or fees received changes
  useEffect(() => {
    const courseFees = parseFloat(calculationFields.courseFees) || 0;
    const discountAmount = parseFloat(formData.discountAmount) || 0;
    const discountRate = formData.discountRate;
    const feesReceived = parseFloat(calculationFields.feesReceived) || 0;

    let totalFees = courseFees;
    if (discountRate === 'Percent') {
      totalFees = courseFees - (courseFees * discountAmount / 100);
    } else if (discountRate === 'Fixed') {
  ...
  }, [calculationFields.courseFees, formData.discountAmount, formData.discountRate, calculationFields.feesReceived]);
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...formData,
      courseFees: parseFloat(calculationFields.courseFees) || 0,
      feesReceived: parseFloat(calculationFields.feesReceived) || 0,
      totalFees: parseFloat(calculationFields.totalFees) || 0,
      balance: parseFloat(calculationFields.balance) || 0,
      discountAmount: parseFloat(formData.discountAmount) || 0,
      incentive: parseFloat(formData.incentive) || 0,
      courseDuration: parseInt(formData.courseDuration) || 1,
      phoneNo: parseInt(formData.mobile) || 0,
      fatherPhoneNo: parseInt(formData.fathersMobileNo) || 0,
      motherPhoneNo: parseInt(formData.mothersMobileNo) || 0,
    ...
    <div className="space-y-6 p-2">
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-gray-600">Course Fees:</p><p className="text-xl font-bold text-black">₹{calculationFields.courseFees || '0'}</p></div>
          <div><p className="text-gray-600">Discount ({formData.discountRate === 'Percent' ? '%' : '₹'}):</p><p className="text-xl font-bold text-red-600">-₹{formData.discountAmount || '0'}</p></div>
          <div><p className="text-gray-600">Total Fees:</p><p className="text-xl font-bold text-green-600">₹{calculationFields.totalFees || '0'}</p></div>
          <div><p className="text-gray-600">Fees Received:</p><p className="text-xl font-bold text-blue-600">₹{calculationFields.feesReceived || '0'}</p></div>
          <div className="col-span-2"><p className="text-gray-600">Balance:</p><p className={`text-2xl font-bold ${parseFloat(calculationFields.balance || 0) > 0 ? 'text-red-600' : 'text-green-600'}`}>₹{calculationFields.balance || '0'}</p></div>
        </div>
      </div>
            ...
            {formData.installments.map((inst, idx) => (
              <div key={idx} className="bg-white border rounded-lg p-4">
                <div className="flex justify-between">
                  ...
                  <div className="text-right"><p className="text-lg font-bold text-green-600">₹{inst.amount}</p><p className="text-sm text-gray-600">Mode: {inst.mode || 'N/A'}</p><p className="text-sm text-gray-600">Status: {inst.paymentStatus}</p></div>
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="space-y-6 mb-8">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <select name="source" value={formData.source} onChange={handleChange} className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500">
                        <option value="">Select...</option>
                        {sourcesData.map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
                      </select>
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    <div>
                      ...
                      <input type="number" name="courseFees" value={calculationFields.courseFees} onChange={handleCalculationChange} className="w-full px-3 py-2 border rounded-md bg-gray-50" readOnly={!initialData} />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <textarea name="remarks" value={formData.remarks} onChange={handleChange} rows="2" className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500" placeholder="Additional remarks"></textarea>
                    </div>
                    <div>
