...
const RoleExamPage = () => {
  ...
  useEffect(() => {
    const fetchRoleExam = async () => {
      setLoading(true);
      try {
        const response = await axiosPrivate.get(`/examinations/role/${roleId}`);
        ...
        if (result && result.data) {
          ...
          const savedAnswers = sessionStorage.getItem(`role_exam_${roleId}_answers`);
          if (savedAnswers) {
            setAnswers(JSON.parse(savedAnswers));
          }

          const savedFlagged = sessionStorage.getItem(`role_exam_${roleId}_flagged`);
          if (savedFlagged) {
            setFlaggedQuestions(JSON.parse(savedFlagged));
          }
        } else {
          toast.error("Failed to load exam data");
          navigate("/teacher/assigned-role");
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (roleId && Object.keys(answers).length > 0 && examStarted) {
      sessionStorage.setItem(`role_exam_${roleId}_answers`, JSON.stringify(answers));
    }
  }, [answers, roleId, examStarted]);

  // Save flagged questions to session storage
  useEffect(() => {
    if (roleId && Object.keys(flaggedQuestions).length > 0 && examStarted) {
      sessionStorage.setItem(`role_exam_${roleId}_flagged`, JSON.stringify(flaggedQuestions));
    }
  }, [flaggedQuestions, roleId, examStarted]);
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        sessionStorage.removeItem(`role_exam_${roleId}_answers`);
        sessionStorage.removeItem(`role_exam_${roleId}_flagged`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate("/teacher/assigned-role")}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          >
            Back to Assigned Roles
          </button>
        </div>
      </div>
    );
          ...
          <div className="space-y-4 mb-8">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-blue-50 p-4 rounded-lg text-center">
                ...
                <p className="text-lg font-bold text-blue-600">{roleData.roleName || 'N/A'}</p>
              </div>
              <div className="bg-green-50 p-4 rounded-lg text-center">
                <p className="text-sm text-gray-600">Course</p>
                <p className="text-lg font-bold text-green-600">{roleData.courseName || 'N/A'}</p>
              </div>
