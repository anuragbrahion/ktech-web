...
const GoalExamRequest = () => {
  ...
  const requestGoalsListData = useSelector(state => state.examination?.requestGoalsListData);
  const requestGoalsUpdateStatusData = useSelector(state => state.examination?.requestGoalsUpdateStatusData);
  const requestGoalsAssignedData = useSelector(state => state.examination?.requestGoalsAssignedData);
  ...
  const fetchRequests = () => {
    ...
    const params = {
      page: currentPage,
      size: itemsPerPage,
      goalId:"690e2e0b6fa7f65963c2c1b2"
    };
  ...
  const tableHeaders = ['S.No', 'Teacher Name', 'E-mail', 'Goals', 'Total Days', 'Status', 'Approve', 'Reject'];

  return (
    <div className="">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Goals Exam Requests</h1>
        <p className="text-gray-600 mt-2">Manage teacher goals examination requests</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        ...
        {filteredRequests.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">No goal exam requests found</h3>
            <p className="text-gray-500">
              {filter !== 'all' 
                ? `No ${filter.toLowerCase()} requests available` 
                : "No goal examination requests available."}
            </p>
          </div>
        )}
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">Loading goal requests...</p>
          </div>
        )}
      </div>
