...
const ExamPage = () => {
  ...
  useEffect(() => {
    const fetchExamQuestions = async () => {
      ...
      try {
        const response = await axiosPrivate.get(`/examinations/student/${examId}`);
  ...
  useEffect(() => {
    if (examId && Object.keys(answers).length > 0 && examStarted) {
  ...
  useEffect(() => {
    if (examId && Object.keys(flaggedQuestions).length > 0 && examStarted) {
      sessionStorage.setItem(`exam_${examId}_flagged`, JSON.stringify(flaggedQuestions));
    }
  }, [flaggedQuestions, examId, examStarted]);

  // Fullscreen and security measures
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden && examStarted && !isSubmitting) {
        setViolationCount((prev) => {
          ...
          if (newCount >= 3) {
            toast.error(
              "You have exceeded the maximum allowed tab switches. Exam will be auto-submitted."
            );
            handleAutoSubmit();
          } else {
            toast.warning(
              `Tab switch detected! ${3 - newCount} violation(s) remaining.`
            );
          }
          return newCount;
        });
    ...
    const handleKeyDown = (e) => {
      ...
      // Disable Alt+Tab detection (warning only)
      if (e.altKey && e.key === "Tab") {
        e.preventDefault();
        toast.warning("Alt+Tab is not allowed during the exam");
        return false;
      }
    };
    ...
    try {
      ...
      for (let i = 0; i < totalQuestions; i++) {
        if (answers[i]) {
          ...
          studentAnswers.push(""); // Send blank for unanswered questions
        }
      }

      // Prepare submission payload as per API requirement
      ...
      if (response?.data) {
        ...
        // Navigate to results page or exams list
        navigate("/student/my-exams", {
          state: { 
  ...
  // Exam not started yet - Show instructions/start screen
  if (!examStarted) {
    return (
          ...
          <div className="space-y-4 mb-8">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-purple-50 p-4 rounded-lg text-center">
                ...
                <p className="text-2xl font-bold text-purple-600">{examData.passingPercentage}%</p>
              </div>
