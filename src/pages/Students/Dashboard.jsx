...
import { getStudentOverview } from "../../redux/slices/studentSlice";

function AdmissionCard({ item }) {
  const isNew = item.type === "New-Admission";
  const formattedDate = formatDateForTable(item.admissionDate);
    ...
    <div className="bg-white rounded-2xl border border-green-100 shadow-sm hover:shadow-md hover:border-green-300 transition-all duration-200 overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs font-mono text-gray-400">{item.rollNo}</span>
        </div>
...
const MyTasks = () => {
  ...
  const fetchOverview = () => {
    ...
    dispatch(getStudentOverview()).then((action) => {
      if (action.error) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {overviewData.map((item) => (
          <AdmissionCard key={item._id} item={item} />
        ))}
      </div>
    </>
  );
