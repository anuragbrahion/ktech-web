...
const StudentFeeSummary = ({ adminId }) => {
  ...
  useEffect(() => {
    if (selectedStudent) {
      setLoading(true);
      dispatch(getAllStuCourseName({ id: selectedStudent }))
        .then((res) => {
          if (res) {
  ...
  const getSelectedCourseData = () => {
    return (
      studentCourses.find((course) => course._id === selectedCourse) || null
    );
  };
  ...
  const getCurrentFeeData = () => {
    ...
    if (
      !selectedCourse ||
      !selectedCourseData ||
      !selectedCourseData.installments
    ) {
      return {
        totalInstallments: 0,
        totalPaid: 0,
    ...
    const totalPaid = installments.reduce(
      (sum, item) => sum + (item.paymentStatus === "Paid" ? item.amount : 0),
      0,
    );
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-sm font-medium ${config.text}`}
      >
        {config.label}
      </span>
    );
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            ...
            <>
              <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-medium text-black">
                      {selectedCourseData.course?.courseName}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-8">
                <div className="flex items-center mb-4">
                  ...
                  <h2 className="text-xl font-bold text-sky-600 mx-4">
                    Fee Summary
                  </h2>
                  <div className="h-px flex-1 bg-gray-300"></div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-green-50 p-6 rounded-lg shadow border border-green-100">
                    ...
                    <p className="text-3xl font-bold text-black">
                      {formatCurrency(feeData.totalPaid)}
                    </p>
                  ...
                  <div
                    className={`p-6 rounded-lg shadow border ${feeData.pendingAmount > 0 ? "bg-red-50 border-red-100" : "bg-gray-50 border-gray-100"}`}
                  >
                    <h3
                      className={`text-lg font-semibold ${feeData.pendingAmount > 0 ? "text-red-700" : "text-gray-700"} mb-2`}
                    >
                      Pending Amount
                    </h3>
                    <p className="text-3xl font-bold text-black">
                      {formatCurrency(feeData.pendingAmount)}
                    </p>
                    <p className="text-sm text-gray-600 mt-1">
                      {feeData.totalFees > 0
                        ? `${Math.round((feeData.pendingAmount / feeData.totalFees) * 100)}% remaining`
                        : "No fees"}
                    </p>
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="py-3 px-4 text-left text-black font-semibold border-b">
                          Total Installments
                        </th>
                        <th className="py-3 px-4 text-left text-black font-semibold border-b">
                          Total Paid
                ...
                {feeData.installments.length > 0 ? (
                  ...
                  <div className="text-center py-8 border border-gray-200 rounded-lg">
                    <p className="text-gray-500">
                      No installment details available for this course.
                    </p>
                    <p className="text-gray-400 text-sm mt-2">
                      All installments for this course are empty.
                    </p>
                  </div>
                )}
              </div>
          ...
          {selectedStudent &&
            ...
            !loading && (
              <div className="text-center py-8 border border-gray-200 rounded-lg">
                <p className="text-gray-500">
                  Please select a course to view fee details.
                </p>
              </div>
            )}
        </div>
